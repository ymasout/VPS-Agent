...
当前进度：M0–M5 已完成，M4 的重启、部署与显式回滚均通过生产验证。M6.1 备份/恢复、M6.2 PWA/移动体验、M6.3 多通道通知、M6.4a 源码发行门、M6.4b Principal 只读授权、M6.4c1 写身份 shadow 与 M6.4c2 具名 operator 计划创建均已完成相应审计、CI 和生产门；详细证据见项目状态与路线图。生产现运行 `c78ae35`，Alembic head 为 `0020_m6_named_approval`，Principal flags 已还原关闭。c3 具名确认与 maker-checker 尚未实现，因此 enforcement 模式下确认必须失败关闭。拉源码/构建、清理、Web SSH 与任意 Shell 仍不在当前范围。原始项目计划书作为产品基线保留，实际进度以项目状态和路线图为准。
