...
当前进度：M0 项目骨架、M1 机器可见、M2 异常可通知、M3 上下文与 AI 诊断和 M5 诊断与操作会话体验均已完成；M4 安全处置核心完成（重启、部署、回滚均生产验证）。M3 的真实仓库文件诊断引用与 `http_json` Provider 两项收尾生产门已于 2026-07-26 通过。审计时生产运行 `ff4f5bc`，诊断 Provider 已还原为 deterministic，已验证的 M5 扩展开关均已还原关闭；当前迁移 head 为 `0017_m5_runbook_drafts`。M6.1“可验证的控制平面备份与离线恢复基础”已完成本地实现和真实 PostgreSQL 16 正/负向验证，等待独立审计；尚未提交、推送、部署或触碰生产。拉源码/构建、清理和 Shell 为后续扩展。首个通知通道采用钉钉自定义机器人。原始项目计划书作为产品基线保留，实际进度以项目状态和路线图为准。
