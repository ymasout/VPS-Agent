...
> 历史归档：本文保留 M3 金丝雀当时的生产证据和旧路径，不再代表当前部署流程。M4.1 已改为 Alembic 显式迁移和 `deploy/caddy/` 目录挂载；当前命令见 [`deploy/README.md`](./README.md)。
...
本清单已在生产金丝雀完整跑通：停止 canary 后形成 Firing 并收到钉钉异常卡；从事件触发诊断后，Agent 经 Caddy 成功领取和完成证据请求，没有 401；证据经过 Agent 与控制平面双重脱敏，`fake-secret` 持久化计数为 0，`[REDACTED]` 计数为 100；诊断进入 Completed，并生成 4 条带证据引用的事实。重启 canary 后事件进入 Resolved，钉钉收到恢复卡。

生产实证覆盖了本批全部安全边界：控制平面不能指定容器目标、Agent 本地白名单、`docker logs` 固定参数和 `--` 分隔、双端脱敏、大小/行数/超时限制、Caddy Bearer 路由、诊断只读不执行，以及结构化输出与证据引用。
...
- [x] 控制平面升级后 M3 的 10 张表已建，4 台 Agent（含仍是 v0.2.4 的）持续上报 200。
- [x] Caddyfile validate 通过，evidence-requests 经 Caddy 不返回 401。
- [x] 至少一台 Agent 升级到 v0.3.0 并正确声明证据源。
- [x] 一个真实服务完成映射（目录/日志源/仓库/版本）。
- [x] 一次真实 Firing 事件能走完诊断闭环：pending -> running -> completed，证据带来源、已脱敏、有大小限制。
- [x] 诊断链路无任意 Shell、任意路径读取或写操作入口。
- [x] M1/M2 监控与告警行为未受影响。
...
2026-07-19 后续产品化首批已在本地实现并通过隔离 PostgreSQL/Compose 验证，但尚未发布或部署：新安装命令可明确选择 Docker 诊断策略，Agent 自动生成稳定身份和来源关联，机器详情页可确认映射；旧容器 ID 的活动事件和既有映射具备兼容迁移。下一次生产动作应使用新的独立金丝雀清单，不重复本文件的手工白名单步骤。
