...
import { POST } from "./dingtalk/route";
...
describe("notification test proxy", () => {
  ...
  it("rejects cross-origin, malformed idempotency, and request bodies", async () => {
    const crossOrigin = await POST(new NextRequest("https://ops.example.com/console/notification-tests/dingtalk", {
      method: "POST",
      headers: { host: "ops.example.com", origin: "https://evil.example", "idempotency-key": id },
    }));
    const malformed = await POST(new NextRequest("https://ops.example.com/console/notification-tests/dingtalk", {
      method: "POST",
      headers: { host: "ops.example.com", origin: "https://ops.example.com", "idempotency-key": "bad" },
    }));
    const body = await POST(new NextRequest("https://ops.example.com/console/notification-tests/dingtalk", {
      method: "POST",
