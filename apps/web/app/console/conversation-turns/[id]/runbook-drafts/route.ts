...
export async function POST(
  ...
  if (!adminToken) {
    return NextResponse.json({ detail: "Runbook drafts are not configured" }, { status: 503 });
  }
  ...
  try {
    const response = await fetch(`${apiURL}/api/v1/conversation-turns/${id}/runbook-drafts`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-admin-token": adminToken },
      body: JSON.stringify(payload),
      cache: "no-store",
    });
    const result = await response.json().catch(() => ({ detail: "control plane rejected the request" }));
