...
describe("repository conversation route", () => {
  it("keeps the admin token server-side and forwards the bounded question", async () => {
    ...
    const request = new NextRequest(
      "https://ops.example.com/console/repositories/repository-1/conversation/turns",
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          question: "compose.yaml 如何配置健康检查？",
        }),
      },
    );

    const response = await POST(request, {
      params: Promise.resolve({ id: "repository-1" }),
    });

    expect(response.status).toBe(202);
    expect(internalFetch).toHaveBeenCalledWith(
      "http://localhost:8000/api/v1/repositories/repository-1/conversation/turns",
  ...
  it("rejects cross-origin, malformed identifiers, and oversized bodies", async () => {
    ...
    const malformed = new NextRequest(
      "https://ops.example.com/console/repositories/bad/conversation/turns",
      {
        method: "POST",
        headers: {
          host: "ops.example.com",
