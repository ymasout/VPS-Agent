...
export async function POST(request: NextRequest) {
  ...
  if (!adminToken) return NextResponse.json({ detail: "deployment planning is not configured" }, { status: 503 });
  ...
  try {
    const response = await fetch(`${apiURL}/api/v1/deployment-plans`, { method: "POST", headers: { "content-type": "application/json", "x-admin-token": adminToken }, body: JSON.stringify(body), cache: "no-store" });
