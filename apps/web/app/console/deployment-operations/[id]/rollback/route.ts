...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!adminToken) return NextResponse.json({ detail: "deployment rollback is not configured" }, { status: 503 });
  const { id } = await context.params;
  if (!/^[A-Za-z0-9-]{1,64}$/.test(id)) return NextResponse.json({ detail: "invalid operation id" }, { status: 400 });
  const apiURL = process.env.API_INTERNAL_URL ?? "http://localhost:8000";
  try {
    const response = await fetch(`${apiURL}/api/v1/deployment-operations/${encodeURIComponent(id)}/rollback`, { method: "POST", headers: { "content-type": "application/json", "x-admin-token": adminToken }, body: JSON.stringify({ expires_in_seconds: 300 }), cache: "no-store" });
