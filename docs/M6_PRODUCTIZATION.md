...
本文冻结 M6 的目标、安全边界和阶段顺序。当前状态为：**M6 已完成（v0.6.1 于 2026-08-01 正式发行并通过 digest-pinned 生产升级金丝雀，2026-08-02 文档收口）**。后续批次 A 的 Agent 安全升级/失败回退、签名 bundle 暂存和持续发行安全门已完成本地实现、验证与独立审计并已本地提交，待推送后 CI 与单独金丝雀；批次 B 的完整灾备运行手册仍仅有设计。两者沿用 M6.1c/d 名称但不重新打开 M6，也不宣称已发布或生产完成。本文不是后续生产操作授权；任何生产备份恢复、外部测试消息或数据修改仍需用户另行明确授权。
...
- 审计开始时 Agent 安装器只能用 SHA-256 发现传输损坏，升级会直接覆盖文件且没有 last-known-good；M6.4d 已增加固定发布者 Sigstore 验证，后续批次 A 又完成事务式升级、精确升级来源、boot recovery 和失败回退的本地实现、验证与独立审计，仍待推送后 CI 和生产金丝雀。
...
- 后续批次 A 已实现 last-known-good 二进制/配置、启动稳定窗口和失败回退；回退明确不包含或删除身份、幂等账本和 machine-id。该能力仍待推送后 CI 与生产金丝雀，不能视为已发布。
...
3. **M6.1c 发布与升级可靠性（M6 后续加固）**：不可变控制平面镜像、依赖镜像固定、统一 CI、首条版本兼容矩阵，以及 bundle 内确定性生成 `images.env` 已完成；Agent last-known-good 升级/回退与已签名 bundle 安全暂存已完成本地实现、验证、独立审计和本地提交，待推送后 CI 和单独金丝雀。
...
3. **部分关闭：**Agent last-known-good、失败回退和精确升级来源门属于 M6.1c；本地实现、验证、独立审计和本地提交已完成，但尚待推送后 CI、发布和生产验证。现有 Agent `v0.4.2` 到控制平面 v0.6.1 的既有协议路径已验证。
...
- 控制平面具备可重复的固定版本安装、升级、兼容检查、应用回退与审计流程；v0.6.1 的正式发行和生产升级路径已通过。Agent `v0.4.2` 兼容范围已经矩阵验证；Agent last-known-good/失败回退是已完成本地实现、验证与独立审计并已本地提交、尚待推送后 CI/金丝雀的后续 M6.1c 加固。
