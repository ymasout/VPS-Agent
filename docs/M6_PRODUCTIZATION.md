...
本文冻结 M6 的目标、安全边界和阶段顺序。当前状态为：**M6 已完成（v0.6.1 于 2026-08-01 正式发行并通过 digest-pinned 生产升级金丝雀，2026-08-02 文档收口）；批次 A 以 v0.6.3 于 2026-08-04 正式发行（零 HIGH/CRITICAL）**。批次 A 的 Agent 安全升级/失败回退、签名 bundle 暂存、四平台候选镜像漏洞扫描和镜像瘦身已实现、审计、通过 CI 并随 v0.6.3 发布；v0.6.3 生产升级与 Agent 事务式升级金丝雀为独立授权事件，尚未执行。批次 B 的完整灾备运行手册仍仅有设计。两者沿用 M6.1c/d 名称但不重新打开 M6，也不宣称已生产完成。本文不是后续生产操作授权；任何生产备份恢复、外部测试消息或数据修改仍需用户另行明确授权。
