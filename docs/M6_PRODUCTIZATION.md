...
本文冻结 M6 的目标、安全边界和阶段顺序。当前状态为：**M6.1、M6.2、M6.3a、M6.3b 已完成相应生产金丝雀；M6.3 整体未完成（M6.3c/d 待开始）**。本文不是后续生产操作授权；任何生产备份恢复、外部测试消息或数据修改仍需用户另行明确授权。
...
- Alembic 是当前 schema 演进唯一入口。生产现运行 `0018_m6_notification_tests`（M6.3b 迁移已应用）。API 启动时校验数据库 revision，不自动迁移。
...
6. **M6.3 通知与引导式配置**：M6.3a 已完成（秘密不回显的就绪状态、固定模板目录、只读引导页，生产金丝雀通过 2026-07-28）；M6.3b 已完成（默认关闭、限速、幂等、最多一次发送的审计测试消息，生产金丝雀通过 2026-07-28）。统一适配器和第二通道（M6.3c/d）后续进入。完整边界见 [M6_NOTIFICATIONS.md](./M6_NOTIFICATIONS.md)。
