...
7. **M6.4 协作与开源评估**：第一轮设计审计已完成，代码尚未开始；先建立秘密安全源码发行门，再定义服务端可信 actor、只读角色和具名审批，最后完善正式发行。SaaS 继续冻结，完整计划见 [M6_COLLABORATION_OPEN_SOURCE.md](./M6_COLLABORATION_OPEN_SOURCE.md)。
