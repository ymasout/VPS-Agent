...
当前生产 Fleet 中，control-plane 自监控、两台腾讯云和 aliyun-VPS 已运行 Agent `v0.4.0`；其中只有 aliyun-VPS 明确开启 `docker_restart`，其余机器没有首轮 M4 写权限。DMIT 保持 `v0.3.3`，与 v0.4.0 控制平面兼容且无需为了首轮 M4 单独升级。v0.4.0 Agent 在策略 disabled 时会初始化本地账本并以默认 5s 轮询空操作队列（无任务、不执行写操作），属可接受的额外空轮询，而非零行为变更。
...
M4.2a 已完成本地实现、自动测试和真实 PostgreSQL/Compose 隔离验证：独立候选表、`AGENT_DEPLOY_POLICY=plan_only`、两步 RepoDigest 采集、完整仓库校验、永久不可执行的 `plan_snapshot` 计划，以及所有现有写入口对 deploy action 的显式拒绝均通过。空库和模拟生产 `0006 -> 0007` 两条迁移路径通过 schema check；真实 Compose canary 完成候选上报、non-critical 映射和冻结计划，确认接口返回 409。M4.2a 尚未部署生产，也没有协议 v2、签名部署任务、pull、Compose 重建或回滚路径；完整证据见 [M4.2_DEPLOY.md §9](./M4.2_DEPLOY.md#9-m42a-本地隔离验证2026-07-22)。
