...
当前生产 Fleet 中，control-plane 自监控、两台腾讯云运行 Agent `v0.4.0`，aliyun-VPS 为 `v0.4.1`（M4.2a `plan_only` + `docker_restart`）；其中只有 aliyun-VPS 明确开启写能力，其余机器没有首轮 M4 写权限。DMIT 保持 `v0.3.3`，与 v0.4.0 控制平面兼容且无需为了首轮 M4 单独升级。v0.4.0 Agent 在策略 disabled 时会初始化本地账本并以默认 5s 轮询空操作队列（无任务、不执行写操作），属可接受的额外空轮询，而非零行为变更。
...
M4.1 已在生产控制平面落地：先在实现提交 `dbb237b` 构建并部署 API/Web 镜像，运行 `sh deploy/control-plane-release.sh adopt` 一次性接管旧 `create_all` 库（`pg_dump` 备份 -> `verify-adoption` 严格结构校验通过 -> `stamp head` 到 `0006` -> `upgrade head` -> `check` 复核），随后 preflight / migrate / `up -d` / reload-caddy / postflight 全部通过。postflight 确认 revision/schema 一致、`/healthz` 200、Agent operation 路由 200（非 401）、服务映射候选 200；Caddy 已切到 `deploy/caddy/` 目录挂载。生产宿主机在 M4.1 验证当时仓库 HEAD 为文档收尾提交 `632ad10`，运行代码对应 `dbb237b` 的 M4.1 实现（后续 M4.2a 已升级到 `8be69ee`/`ebf0163`）。M4 写闭环回归：在 aliyun-VPS 重起 canary，操作 `queued -> claimed -> running -> verifying -> succeeded`、8 次审计转换、`output` 不含容器 target、`succeeded` 由独立健康观测判定；该 canary 当前继续运行，保留为 M4.2 测试候选。手动 ALTER 与单文件 Caddy 挂载自此成为历史，标准发布改为显式 Alembic 迁移 + 目录挂载。
...
M4.2a 已完成本地实现、自动测试和真实 PostgreSQL/Compose 隔离验证：独立候选表、`AGENT_DEPLOY_POLICY=plan_only`、两步 RepoDigest 采集、完整仓库校验、永久不可执行的 `plan_snapshot` 计划，以及所有现有写入口对 deploy action 的显式拒绝均通过。空库和模拟生产 `0006 -> 0007` 两条迁移路径通过 schema check；真实 Compose canary 完成候选上报、non-critical 映射和冻结计划，确认接口返回 409。M4.2a 已于 2026-07-22 通过生产金丝雀（aliyun-VPS v0.4.1 `plan-only` 发现候选、冻结计划永久不可执行、确认接口 409），但生产环境仍无协议 v2、签名部署任务、pull、Compose 重建或回滚路径；完整证据见 [M4.2_DEPLOY.md §9–10](./M4.2_DEPLOY.md)。

M4.2b 已完成本地实现、真实隔离验证和生产金丝雀：`0008`、协议 v2、双 digest 签名、Agent/控制平面双重显式授权、允许目录与双基线 config hash、固定单服务 Compose 重建、同报告 digest+健康验证均落地；真实 A/B 往返成功、unhealthy 超时失败、同服务并发 409；2026-07-23 生产 A->B 部署 `succeeded`（同报告 digest+健康验证）。证据见 [M4.2_DEPLOY.md §11、§13](./M4.2_DEPLOY.md)。

M4.2c 已完成本地实现、真实隔离验证和生产金丝雀：`0009` 增加 `rollback_of`，失败部署不会自动触发第二次写；管理员显式创建的回滚计划只允许恢复原计划冻结的旧 digest，重新预检、再次确认并复用协议 v2 独立执行和验证。真实 A->坏 B->A 闭环、8 次回滚审计转换和恢复后的漂移 409 均通过；2026-07-23 生产 A->坏B `verification_timeout` 失败 -> 显式回滚 `succeeded`（目标服务端派生、不健康可恢复）。证据见 [M4.2_DEPLOY.md §12、§13](./M4.2_DEPLOY.md)。

**M4 核心（重启 + 部署 + 回滚）至此生产验证完成。** 剩余 M4 范围（拉源码/构建、受限清理）为后续扩展；自然语言入口属 M5。
