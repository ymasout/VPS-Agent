...
M4.2b 已完成本地实现和真实隔离验证：`0008`、协议 v2、双 digest 签名、Agent/控制平面双重显式授权、允许目录与双基线 config hash、固定单服务 Compose 重建、同报告 digest+健康验证均落地；真实 A/B 往返成功、unhealthy 超时失败、同服务并发 409。生产仍保持 M4.2a 状态，尚未发布或启用部署执行；证据与生产前边界见 [M4.2_DEPLOY.md §11](./M4.2_DEPLOY.md)。

M4.2c 已完成本地实现和真实隔离验证：`0009` 增加 `rollback_of`，失败部署不会自动触发第二次写；管理员显式创建的回滚计划只允许恢复原计划冻结的旧 digest，重新预检、再次确认并复用协议 v2 独立执行和验证。真实 A->坏 B->A 闭环、8 次回滚审计转换和恢复后的漂移 409 均通过。生产仍保持 M4.2a，M4.2b/M4.2c 尚未发布；证据见 [M4.2_DEPLOY.md §12](./M4.2_DEPLOY.md)。
