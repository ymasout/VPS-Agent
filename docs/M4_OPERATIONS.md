...
本文 §1–8 记录 M4 第一条纵向闭环（Docker 安全重启）。M4.2 部署与回滚能力见 §12。当前不提供 Pull 源码、systemd 写操作、受限清理、任意命令、任意参数、任意路径或 Shell。
...
M4.2a 已完成本地实现、自动测试和真实 PostgreSQL/Compose 隔离验证：独立候选表、`AGENT_DEPLOY_POLICY=plan_only`、两步 RepoDigest 采集、完整仓库校验、永久不可执行的 `plan_snapshot` 计划，以及所有现有写入口对 deploy action 的显式拒绝均通过。空库和模拟生产 `0006 -> 0007` 两条迁移路径通过 schema check；真实 Compose canary 完成候选上报、non-critical 映射和冻结计划，确认接口返回 409。M4.2a 已于 2026-07-22 通过生产金丝雀（在 M4.2a 验证当时 aliyun-VPS 为 v0.4.1 `plan-only`，发现候选、冻结计划永久不可执行、确认接口 409），但生产环境仍无协议 v2、签名部署任务、pull、Compose 重建或回滚路径；完整证据见 [M4.2_DEPLOY.md §9–10](./M4.2_DEPLOY.md)。
