...
- `v0.2.3` Release 已发布并完成双架构、校验和、安装器下载源参数和版本输出验证。
- Agent 注册支持使用新的一次性令牌恢复身份文件丢失的离线机器，同时拒绝覆盖仍在线的相同 machine-id。
- 新安装由安装器生成独立持久的 Agent machine-id，规避云镜像克隆造成的 `/etc/machine-id` 重复，同时不修改操作系统标识。
- `v0.2.4` Release 已发布并验证，Fleet 页脚增加 Console 版本用于确认 Web 容器是否完成更新。

### 尚未完成的验收

- 接入并稳定运行 3 台真实或测试 VPS。
- 将修正后的 Agent 发布到真实主机并复核 systemd 运行、停止和失败三类状态。
- 使用 `v0.2.3` Release 和控制平面下载中转完成国内 VPS 的实机安装验证。
