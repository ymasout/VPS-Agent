...
当前自动验证：API 102 项测试、Web 26 项测试、全部 Go 包测试、Ruff、ESLint、`go vet` 和 Web 生产构建通过。真实隔离栈包含 PostgreSQL、Redis、Caddy、API、Web、Agent 和非关键 Docker canary；已验证确认前不可领取、健康重启成功、命令成功但 unhealthy 最终失败、重复结果幂等、同服务并发 409、策略关闭拒绝，以及 Agent 离线和 API 在 Claimed/Running/Verifying 状态重启后的保守恢复。M4 仍保持进行中（首轮安全重启已生产金丝雀通过，后续拉取/部署/回滚/清理未开始）。2026-07-20/21 生产金丝雀在 aliyun VPS（新机、有 Docker）跑通：M4 首批通过提交前安全审计（有条件通过、全部 P2/P3 已处理），提交 `84cb4a2` 推送至 `main` 并发布 `v0.4.0` Release；金丝雀 Agent 以 `--operation-policy docker-restart` 安装，`m4-canary` 映射为 non_critical + restart_enabled，端到端 `queued -> claimed -> running -> verifying -> succeeded`、审计 8 次转换完整、`output` 不含容器 target、`succeeded` 由独立健康观测判定。部署坑（0006 加列需手动 ALTER、Caddyfile 变更需 reload）见 [M4_OPERATIONS.md §10](./M4_OPERATIONS.md)。非 Docker 的 VPS 无需为了首轮 M4 单独升级（旧 v0.3.x Agent 与新控制平面兼容）。
