...
最后同步：2026-07-21
当前阶段：**M0、M1、M2 已完成；M3、M4 进行中（M4 首轮安全重启已生产金丝雀通过）**

## 1. 当前结论

项目已完成工程骨架、“机器可见”和“异常可通知”里程碑。生产控制平面通过 Caddy/HTTPS 运行，Agent 使用一次性令牌注册、独立凭证认证和主动出站 HTTPS 上报；服务异常、去重、钉钉通知和恢复通知已经过生产杀手路径验证。

当前共有 5 条真实 VPS 机器记录。M1 验收时，3 台外部 VPS 和控制平面宿主机均通过 Release 安装器运行 Agent `v0.2.4`；M3 生产验证后，control-plane 为 `v0.3.1`、DMIT 为 `v0.3.3`、两台腾讯云仍为 `v0.2.4`，均保留原身份；M4 生产金丝雀新增 aliyun-VPS（`v0.4.0`、`docker-restart`，金丝雀容器已清理、Agent 保留为第 5 台长期监控机）。M1 的“至少 3 台真实或测试 VPS 稳定接入”验收线已经满足。
...
- 2026-07-21 M4.1 生产验证通过：控制平面升级到 `dbb237b`，`adopt` 一次性接管旧 `create_all` 库（备份 + 严格结构校验 + `stamp head` 到 0006），preflight/migrate/部署/postflight 全部通过；postflight 校验 revision/schema、`/healthz`、Agent operation 路由（非 401）和映射候选接口；Caddy 切到 `deploy/caddy/` 目录挂载。M4 写闭环回归：aliyun-VPS canary `queued -> succeeded`、8 次审计转换、独立健康验证通过。

当前自动验证：API 103 项测试、Web 26 项测试、全部 Go 包测试、Ruff、ESLint、`go vet` 和 Web 生产构建通过。真实隔离栈包含 PostgreSQL、Redis、Caddy、API、Web、Agent 和非关键 Docker canary；已验证确认前不可领取、健康重启成功、命令成功但 unhealthy 最终失败、重复结果幂等、同服务并发 409、策略关闭拒绝，以及 Agent 离线和 API 在 Claimed/Running/Verifying 状态重启后的保守恢复。M4 仍保持进行中（首轮安全重启已生产金丝雀通过、M4.1 安全发布基础设施已生产验证，后续拉取/部署/回滚/清理未开始）。2026-07-20/21 生产金丝雀在 aliyun VPS（新机、有 Docker）跑通：M4 首批通过提交前安全审计（有条件通过、全部 P2/P3 已处理），提交 `84cb4a2` 推送至 `main` 并发布 `v0.4.0` Release；金丝雀 Agent 以 `--operation-policy docker-restart` 安装，`m4-canary` 映射为 non_critical + restart_enabled，端到端 `queued -> claimed -> running -> verifying -> succeeded`、审计 8 次转换完整、`output` 不含容器 target、`succeeded` 由独立健康观测判定。历史部署坑（0006 加列与单文件 Caddy 挂载）及 M4.1 标准发布路径见 [M4_OPERATIONS.md §10–11](./M4_OPERATIONS.md)。非 Docker 的 VPS 无需为了首轮 M4 单独升级（旧 v0.3.x Agent 与新控制平面兼容）。
