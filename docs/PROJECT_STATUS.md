...
最后同步：2026-07-22
当前阶段：**M0、M1、M2 已完成；M3、M4 进行中（M4 首轮安全重启、M4.1、M4.2a 已生产验证；M4.2b 已完成本地实现与隔离验证，尚未发布生产）**
...
当前自动验证：API 121 项测试、Web 34 项测试、全部 Go 包测试、Ruff、ESLint、`go vet` 和 Web 生产构建通过。M4.2a 的只读发现/冻结计划与 M4.2b 的协议 v2、双 digest 签名、显式部署授权、Compose 本地安全门和独立验证已经并存；历史 plan-only 永久不可执行，旧 Agent 不受影响。M4.2b 真实隔离栈包含 PostgreSQL、Redis、API、Agent 和正/负两个 Compose canary，并覆盖成功往返、不健康失败、并发锁、部署后下一次双基线复现及 `0007 -> 0008` 迁移。既有 M4 重启隔离测试继续覆盖确认门、重复结果幂等、策略关闭、Agent 离线和 API 在 Claimed/Running/Verifying 状态重启后的保守恢复。M4 仍保持进行中：首轮安全重启、M4.1 和 M4.2a 已生产验证；M4.2b 仅本地完成，生产发布/金丝雀与 M4.2c 显式回滚仍待执行。2026-07-20/21 生产金丝雀在 aliyun VPS（新机、有 Docker）跑通：M4 首批通过提交前安全审计（有条件通过、全部 P2/P3 已处理），提交 `84cb4a2` 推送至 `main` 并发布 `v0.4.0` Release；金丝雀 Agent 以 `--operation-policy docker-restart` 安装，`m4-canary` 映射为 non_critical + restart_enabled，端到端 `queued -> claimed -> running -> verifying -> succeeded`、审计 8 次转换完整、`output` 不含容器 target、`succeeded` 由独立健康观测判定。历史部署坑（0006 加列与单文件 Caddy 挂载）及 M4.1 标准发布路径见 [M4_OPERATIONS.md §10–11](./M4_OPERATIONS.md)。当前 control-plane 自监控、两台腾讯云为 `v0.4.0`，aliyun-VPS 为 `v0.4.1`（M4.2a plan-only），DMIT 为兼容运行的 `v0.3.3`；只有 aliyun-VPS 开启首轮 M4 写权限。
