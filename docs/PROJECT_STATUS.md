...
最后同步：2026-07-22
当前阶段：**M0、M1、M2 已完成；M3、M4 进行中（M4 首轮安全重启和 M4.1 已生产验证；M4.2a 已本地实现并通过隔离验证，尚未部署生产）**

## 1. 当前结论

项目已完成工程骨架、“机器可见”和“异常可通知”里程碑。生产控制平面通过 Caddy/HTTPS 运行，Agent 使用一次性令牌注册、独立凭证认证和主动出站 HTTPS 上报；服务异常、去重、钉钉通知和恢复通知已经过生产杀手路径验证。

当前共有 5 条真实 VPS 机器记录。control-plane 自监控、两台腾讯云和 aliyun-VPS 均运行 Agent `v0.4.0`，DMIT 保持 `v0.3.3`；所有机器均保留原身份。只有 aliyun-VPS 明确开启 `docker-restart`，其 `m4-canary` 当前继续运行并保留为 M4.2 测试候选；其余机器没有首轮 M4 写权限。M1 的“至少 3 台真实或测试 VPS 稳定接入”验收线已经满足。
...
当前自动验证：API 115 项测试、Web 29 项测试、全部 Go 包测试、Ruff、ESLint、`go vet` 和 Web 生产构建通过。M4.2a 已实现独立部署候选表、`plan_only` Agent 发现、两步 RepoDigest 解析、同完整仓库校验、永久不可执行计划及写入口纵深拒绝；真实 PostgreSQL 空库和模拟生产 `0006 -> 0007` 两条迁移路径均通过 schema check，真实 Compose canary 完成候选上报、non-critical 映射和冻结计划，确认接口按设计返回 409，隔离资源已清理。既有真实隔离栈包含 PostgreSQL、Redis、Caddy、API、Web、Agent 和非关键 Docker canary；已验证确认前不可领取、健康重启成功、命令成功但 unhealthy 最终失败、重复结果幂等、同服务并发 409、策略关闭拒绝，以及 Agent 离线和 API 在 Claimed/Running/Verifying 状态重启后的保守恢复。M4 仍保持进行中（首轮安全重启已生产金丝雀通过、M4.1 安全发布基础设施已生产验证；M4.2a 本地完成但尚未部署生产，M4.2b 拉取/部署和 M4.2c 回滚未开始）。2026-07-20/21 生产金丝雀在 aliyun VPS（新机、有 Docker）跑通：M4 首批通过提交前安全审计（有条件通过、全部 P2/P3 已处理），提交 `84cb4a2` 推送至 `main` 并发布 `v0.4.0` Release；金丝雀 Agent 以 `--operation-policy docker-restart` 安装，`m4-canary` 映射为 non_critical + restart_enabled，端到端 `queued -> claimed -> running -> verifying -> succeeded`、审计 8 次转换完整、`output` 不含容器 target、`succeeded` 由独立健康观测判定。历史部署坑（0006 加列与单文件 Caddy 挂载）及 M4.1 标准发布路径见 [M4_OPERATIONS.md §10–11](./M4_OPERATIONS.md)。当前 control-plane 自监控、两台腾讯云和 aliyun-VPS 为 `v0.4.0`，DMIT 为兼容运行的 `v0.3.3`；只有 aliyun-VPS 开启首轮 M4 写权限。
