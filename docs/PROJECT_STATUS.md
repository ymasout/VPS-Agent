...
状态：**进行中（M6.1 首片已生产金丝雀通过 2026-07-27；M6.2a/M6.2b `80b950f` 已部署，PWA Phase 3 暴露 standalone `public/` 打包缺口，本地修复待提交/CI/重新部署；M6.3–M6.4 待开始）**
...
M6.2a/M6.2b 已完成本地实现、验证和独立源码审计：新增 manifest、静态图标、受限 service worker、无业务数据离线页、独立 `/mobile` 只读状态页、移动安全区/只读导航，以及展示冻结目标、动作、风险、有效期和前置检查的独立审批卡。审批默认禁用，必须显式勾选且在线，仍仅调用现有同源 M4 确认代理；不新增 API、迁移、Agent 协议、后台同步或离线写。代码 `80b950f` 已部署，生产页脚确认当前构建为 `80b950f64f70`，manifest、图标链接和移动导航已出现；但 PWA Phase 3 实机检查发现 `/sw.js` 返回 Next 404，未形成 SW 注册或预缓存。根因是 `apps/web/Dockerfile` 的 standalone runtime 未复制 `apps/web/public`，源码级测试未覆盖最终镜像内容。当前工作区已补 `COPY .../public`、Dockerfile 回归断言和 Ubuntu CI 真实镜像启动/四资产请求门；Web 80 项、lint 和普通生产构建通过。本机 Docker daemon 未运行，Windows `DOCKER_BUILD=1` 又受 symlink 权限限制，因此真实 Linux 镜像证据必须由新增 CI 首次运行提供。当前修复未提交、未推送、未重新部署；PWA、离线和 M4 移动审批金丝雀均未完成，边界与失败记录见 [M6_PWA_MOBILE.md](./M6_PWA_MOBILE.md)。
